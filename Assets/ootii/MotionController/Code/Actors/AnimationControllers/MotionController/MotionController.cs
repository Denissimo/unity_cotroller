...
namespace com.ootii.Actors.AnimationControllers
{
    ...
    public class MotionController : MonoBehaviour
    {
        ...
        protected void Awake()
        {
            ...
            {
                if (mAnimator.parameters[i].name == MotionController.MOTION_STYLE_NAMES[0])
                {
                    mUseMotionForms = true;
                }

                if (mAnimator.parameters[i].name == MotionController.MOTION_STATE_TIME[0])
                {
        ...
        public void OnControllerLateUpdate(ICharacterController rController, float rDeltaTime, int rUpdateIndex)
        {
            ...
            if (mTargetPosition.sqrMagnitude > 0f)
            {
                ...
                if (lTargetDistance < 0.5f * mTargetNormalizedSpeed)
                {
            ...
            if (_ShowDebug)
            {
                ...
                Log.FileScreenWrite(String.Format("{0} MC.Update() Motion:{1} MotionDur:{2:f4} Phase:{3} Param:{4} State:{5}", name, (GetActiveMotion(1) != null ? GetActiveMotion(1).GetType().Name : "null"), (GetActiveMotion(1) != null ? GetActiveMotion(1).Age : 0), State.AnimatorStates[1].MotionPhase, State.AnimatorStates[1].MotionParameter, AnimatorHashToString(State.AnimatorStates[1].StateInfo, State.AnimatorStates[1].TransitionInfo)), 6);
        ...
        private void SetAnimatorProperties(ref MotionState rState, bool rUseTrendData)
        {
            ...
            for (int i = 0; i < rState.AnimatorStates.Length; i++)
            {
                ...
                if (i < MotionLayers.Count)
                {
                    ...
                    {
                        int lCurrentMotionPhase = mAnimator.GetInteger(MOTION_PHASE_NAMES[i]);
                    ...
                    if (mUseMotionForms)
                    {
                        mAnimator.SetInteger(MOTION_STYLE_NAMES[i], lState.MotionForm);
                    }
