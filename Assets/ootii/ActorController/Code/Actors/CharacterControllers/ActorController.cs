...
//                        }
            ...
            if (_ShowDebug)
            {
                Utilities.Debug.Log.FileScreenWrite(string.Format("isG:{0} gDist:{1:f3} gAng:{2:f3} spd:{3:f3}", mState.IsGrounded, lGroundDistance, mState.GroundSurfaceAngle, (lFinalMovement.magnitude / Time.deltaTime)), 1);
            ...
            if (!lIsGrounded)
            {
                ...
                if (!lIgnore)
                {
                    ...
                    if (lGroundHit)
                    {
                        ...
                        mState.GroundSurfaceAngle = Vector3.Angle(lRaycastHit.normal, rActorUp);
                        //mState.GroundSurfaceAngle = 0f;
                        mState.GroundSurfaceNormal = lRaycastHit.normal;
                        //mState.GroundSurfaceNormal = Vector3.up;
                        mState.GroundSurfaceDistance = lDistance;
                        mState.GroundSurfacePoint = lClosestPoint;
                        mState.GroundSurfaceDirection = lRayDirection;
                        mState.IsGroundSurfaceDirect = false;
