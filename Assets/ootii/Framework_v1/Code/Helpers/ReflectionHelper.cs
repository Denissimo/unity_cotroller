...
namespace com.ootii.Helpers
{
    public class ReflectionHelper
    {
        ...
        /// <param name="rBaseType">Based type we are comparing</param>
        /// <returns></returns>
        public static bool IsSubclassOf(Type rType, Type rBaseType)
        {
            ...
            if (rType == rBaseType || rType.GetTypeInfo().IsAssignableFrom(rBaseType.GetTypeInfo())) { return true; }
#else
        ...
        /// <param name="rBaseType">Based type we are comparing</param>
        /// <returns></returns>
        public static bool IsAssignableFrom(Type rType, Type rBaseType)
        {
#if !UNITY_EDITOR && (NETFX_CORE || WINDOWS_UWP || UNITY_WP8 || UNITY_WP_8_1 || UNITY_WSA || UNITY_WSA_8_0 || UNITY_WSA_8_1 || UNITY_WSA_10_0)
            if (rType == rBaseType || rType.GetTypeInfo().IsAssignableFrom(rBaseType.GetTypeInfo())) { return true; }
#else
            if (rType == rBaseType || rType.IsAssignableFrom(rBaseType)) { return true; }
#endif
            return false;
        }
