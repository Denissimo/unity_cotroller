...
public class BasicInventoryEditor : Editor
{
    ...
    public override void OnInspectorGUI()
    {
        ...
        if (mTarget.EditorShowEvents)
        {
            ...
            SerializedProperty lItemEquippedEvent = mTargetSO.FindProperty("ItemEquippedEvent");
            ...
            SerializedProperty lItemStoredEvent = mTargetSO.FindProperty("ItemStoredEvent");
            ...
            SerializedProperty lWSEquippedEvent = mTargetSO.FindProperty("WeaponSetEquippedEvent");
            if (lWSEquippedEvent != null)
            {
                EditorGUI.BeginChangeCheck();
                EditorGUILayout.PropertyField(lWSEquippedEvent);
                if (EditorGUI.EndChangeCheck())
                {
                    mIsDirty = true;
                }
            ...
            SerializedProperty lWSStoredEvent = mTargetSO.FindProperty("WeaponSetStoredEvent");
            if (lWSStoredEvent != null)
            {
                EditorGUI.BeginChangeCheck();
                EditorGUILayout.PropertyField(lWSStoredEvent);
                if (EditorGUI.EndChangeCheck())
                {
                    mIsDirty = true;
                }
    ...
    private void DrawWeaponSetListItem(Rect rRect, int rIndex, bool rIsActive, bool rIsFocused)
    {
        if (rIndex < mTarget.WeaponSets.Count)
        {
            ...
            string lID = mTarget.WeaponSets[rIndex].ID;

            Rect lNameRect = new Rect(rRect.x, rRect.y, rRect.width, EditorGUIUtility.singleLineHeight);
            EditorGUI.LabelField(lNameRect, lID);
        }
    }
    ...
    private bool DrawWeaponSetDetailItem(BasicInventorySet rWeaponSet)
    {
        ...
        string lID = (rWeaponSet.ID.Length > 0 ? rWeaponSet.ID : "Weapon Set " + (lWeaponSetIndex + 1).ToString());

        EditorHelper.DrawSmallTitle(lID);

        if (EditorHelper.BoolField("Is Enabled", "Determines if the weapon set will be enabled and disabled using input.", rWeaponSet.IsEnabled, mTarget))
        {
            lIsDirty = true;
            rWeaponSet.IsEnabled = EditorHelper.FieldBoolValue;
        }

        if (EditorHelper.TextField("Name", "Unique name for the weapon set", rWeaponSet.ID, mTarget))
        {
            lIsDirty = true;
            rWeaponSet.ID = EditorHelper.FieldStringValue;
        }

        if (EditorHelper.IntField("Stance", "Value we'll set the Actor Core's 'Stance' state value to when equipped. Use a value less than 0 to not set the state value.", rWeaponSet.Stance, mTarget))
        {
            lIsDirty = true;
            rWeaponSet.Stance = EditorHelper.FieldIntValue;
        }

        if (EditorHelper.IntField("Default Form", "Value we'll set the Actor Core's 'Default Form' state value to when equipped. Use a value less than 0 to not set the state value.", rWeaponSet.DefaultForm, mTarget))
        {
            lIsDirty = true;
            rWeaponSet.DefaultForm = EditorHelper.FieldIntValue;
        }
